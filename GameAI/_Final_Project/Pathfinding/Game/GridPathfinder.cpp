...
void GridPathfinder::drawLineToNodes(Node * node)
{
	...
	al_draw_line(gpGameApp->getGameMapManager()->getMap(mMapID)->getGrid()->getULCornerOfSquare(node->getId()).getX() + gpGameApp->getGameMapManager()->getMap(mMapID)->getGrid()->getSquareSize() / 2,
